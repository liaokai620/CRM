...
public class ChatServiceImpl implements IChatService{
    ...
    //配置您申请的KEY
    public static final String APPKEY = "ba3b7e8fffa92f0a0b496f26d97c0659";

    //1.问答
    @Override
    public Object sendMsg(String msg) {
        String result = null;
        String url = "http://way.jd.com/turing/turing";//请求接口地址
        Map params = new HashMap();//请求参数
        params.put("appkey", APPKEY);//您申请到的本接口专用的APPKEY
        params.put("info", msg);//要发送给机器人的内容，不要超过30个字符
        params.put("dtype", "json");//返回的数据的格式，json或xml，默认为json
        params.put("loc", "广州是天河区棠下小码哥");//地点，如北京中关村
        params.put("lon", "");//经度，东经116.234632（小数点后保留6位），需要写为116234632
        params.put("lat", "");//纬度，北纬40.234632（小数点后保留6位），需要写为40234632
        params.put("userid", "222");//1~32位，此userid针对您自己的每一个用户，用于上下文的关联
    ...
    public static void getRequest2() {
        ...
        String url = "http://way.jd.com/turing/turing";//请求接口地址
        Map params = new HashMap();//请求参数
        params.put("dtype", "json");//返回的数据格式，json或xml，默认json
