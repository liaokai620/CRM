<%@ page language="java" contentType="text/html; charset=UTF-8"
         pageEncoding="UTF-8"%>
<!DOCTYPE html>
<html lang="en" class="no-js">

<head>
  <title>Matrix Admin</title>
  <meta charset="UTF-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1.0" />
  <link rel="stylesheet" href="/static/index/css/bootstrap.min.css" />
  <link rel="stylesheet" href="/static/index/css/bootstrap-responsive.min.css" />
  <link rel="stylesheet" href="/static/index/css/matrix-style2.css" />
  <link rel="stylesheet" href="/static/index/css/matrix-media.css" />
  <link href="/static/index/font-awesome/css/font-awesome.css" rel="stylesheet" />
  <link href='http://fonts.googleapis.com/css?family=Open+Sans:400,700,800' rel='stylesheet' type='text/css'>
</head>
<body>



<div id="content">
  <div id="content-header">
    <h1>对不起,签到失败,请稍后再试!</h1>
  </div>
  <div class="container-fluid">
    <div class="row-fluid">
      <div class="span12">
        <div class="widget-box">
          <div class="widget-title"> <span class="icon"> <i class="icon-info-sign"></i> </span>
            <h5>对不起,签到失败,请稍后再试!</h5>
          </div>
          <div class="widget-content">
            <div class="error_ex">
              <h3>对不起,签到失败,请稍后再试!</h3>
              <a class="btn btn-warning btn-big"  href="/main">返回主页</a> </div>
          </div>
        </div>
      </div>
    </div>
  </div>
</div>

<script src="http://www.jq22.com/jquery/jquery-1.7.1.js"></script>
<script src="/static/index/js/jquery.ui.custom.js"></script>
<script src="/static/index/js/bootstrap.min.js"></script>
<script src="/static/index/js/maruti.html"></script>
</body>
</html>